...
def stft(x, fftsize=1024, overlap=4):
    ...
    return np.array([np.fft.rfft(w*x[i:i+int(fftsize)]) for i in range(0, len(x)-int(fftsize)+1, hop)])
...
def istft(X, overlap=4):
    ...
    x = np.zeros((X.shape[0]+1)*hop)
    wsum = np.zeros((X.shape[0]+1)*hop)
...
if __name__ == "__main__":
    ...
    (sr,wf) = wavefile.read('orig_audio.wav')
    wf = wf[10*sr:20*sr]
